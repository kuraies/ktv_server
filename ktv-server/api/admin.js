...
router.post('/music/add', (req, res) => {
    ...
    Music.findOne({songName})
        .then(hasOne => {
            if (!hasOne || hasOne.artist !== req.body.artist) {// 歌曲名称已经存在
                // 是否存在该歌曲的不同翻唱的歌手
                const song = {
                    songName,
                    artist,
...
router.post('/like/add', async (req, res) => {
    ...
    if (typeof _id == "string") {
        Music.findByIdAndUpdate({_id}, {"like": "1"})
            .then(() => {
                return res.json({status: 200, result: '添加成功'})
            })
            .catch(err => {
                console.log('添加歌曲err=>', err)
                return res.status(500).json({status: 500, result: '添加失败，服务器内部错误'})
            })
    } else {
        let updateOps = [];
        _id.forEach((item) => {
        ...
        await Music.bulkWrite(updateOps)
            .then(() => {
                return res.json({status: 200, result: '添加成功'})
            })
            .catch(err => {
...
router.get('/poster', (req, res) => {
    // to_do 判断这个海报是否存在
    let imgPath = `./static/poster/${req.query.img}`;
    if (fs.existsSync(imgPath)) {
        return res.sendFile(path.resolve(__dirname, "../static/poster/" + req.query.img))
    } else {
        return res.status(416).json({status: 416, result: '海报不存在'})
    }
...
//获取所有推荐歌曲
router.get('/alllike', (req, res) => {
    Music.find({"like": 1}).then(musics => {
        if (musics.length > 0) {
            res.send({status: 200, result: musics})
        } else {
            res.json({status: 200, result: '音乐列表为空'})
...
router.post('/searchByName', (req, res) => {
    ...
    Music.find({
        $or: [
            ...
            {artist: {$regex: songName, $options: 'i'}}
        ]
    })
        .then(musics => {
            if (musics.length > 0) {
...
router.post('/searchByNameLike', (req, res) => {
    const songName = req.body.searchName.trim()
    // to_do 加上or的判断 歌手的名称
    Music.find({
        "like": 1,
...
router.post('/like/delete', (req, res) => {
    const _id = req.body._id
    Music.findByIdAndUpdate({_id}, {"like": "0"})
        .then(() => {
            res.json({status: 200, result: '删除成功'})
        })
        .catch(err => {
